		...
		success : function(data) {
			...
			$(data).each(function(i,item) {
				if(item.fYyQty) {
					...
					if(one) {
						option.title.text = '金银花提取液收率分析(' + datetime.substr(0,4) +')';
						one = false;
					}
					// option.xAxis.categories.push(item.sBatchNumber + '  ' + dateFormat(item.tmCyEnd));
					// option.series[0].data.push(item.fZlyTotalVol);
					
					option.xAxis.categories.push( item.sBatchNumber + ' ' + datetime.substring(5));
